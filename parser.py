...
# Symbol table class
class SymbolTable:
    def __init__(self, nng: NewNameGenerator) -> None:
        self.ht_stack = [dict()]
        self.name_counter = 0
        self.nng = nng

    def insert(self, ID: str, id_type: IDType, data_type: Type) -> None:
        ID_name = self.nng.mk_new_name()
        info = SymbolTableData(id_type, data_type, ID_name)
...
class Parser:
    ...
    def parse_assignment_statement_base(self) -> List[str]:
        ...
        if id_data.get_id_type() == IDType.VAR:
            # Handle variable assignment with possible type conversion
            if id_data.get_data_type() != expr_node.node_type:
                if id_data.get_data_type() == Type.FLOAT and expr_node.node_type == Type.INT:
                    # Convert int to float
                    conv_vr = self.vra.mk_new_vr()
                    code.append(f"{conv_vr} = vr_int2float({expr_node.vr});")
                    code.append(f"{id_data.get_new_name()} = {conv_vr};")
                elif id_data.get_data_type() == Type.INT and expr_node.node_type == Type.FLOAT:
                    # Convert float to int
                    conv_vr = self.vra.mk_new_vr()
                    code.append(f"{conv_vr} = vr_float2int({expr_node.vr});")
                    code.append(f"{id_data.get_new_name()} = {conv_vr};")
                else:
                    code.append(f"{id_data.get_new_name()} = {expr_node.vr};")
            else:
                code.append(f"{id_data.get_new_name()} = {expr_node.vr};")
        else:
            # Handle IO assignment with possible type conversion
            if id_data.get_data_type() != expr_node.node_type:
                if id_data.get_data_type() == Type.FLOAT and expr_node.node_type == Type.INT:
                    # Convert int to float
                    conv_vr = self.vra.mk_new_vr()
                    code.append(f"{conv_vr} = vr_int2float({expr_node.vr});")
                    code.append(f"{id_name} = vr2float({conv_vr});")
                elif id_data.get_data_type() == Type.INT and expr_node.node_type == Type.FLOAT:
                    # Convert float to int
                    conv_vr = self.vra.mk_new_vr()
                    code.append(f"{conv_vr} = vr_float2int({expr_node.vr});")
                    code.append(f"{id_name} = vr2int({conv_vr});")
                else:
    ...
    def parse_if_else_statement(self) -> List[str]:
        ...
        else_label = self.nlg.mk_new_label()   # Label for else block
        end_label = self.nlg.mk_new_label()    # Label after both branches

        # Evaluate condition and compare to zero
        cond_code = expr_node.linearize(self.vra)
        zero_vr = self.vra.mk_new_vr()

        code = cond_code
        code.append(f"{zero_vr} = int2vr(0);")
        code.append(f"beq({expr_node.vr}, {zero_vr}, {else_label});")  # Jump to else if false

        # Then block (if)
        stmt1_code = self.parse_statement()
        code.extend(stmt1_code)
        code.append(f"branch({end_label});")
    ...
    def parse_for_statement(self) -> List[str]:
        ...
        code.append(f"{zero_vr} = int2vr(0);")
        code.append(f"beq({cond_node.vr}, {zero_vr}, {end_label});")

        code.append(f"goto {body_label};")
    ...
    def parse_unit(self) -> ASTNode:
        ...
        if token_id in [Token.ID]:
            ...
            if id_data.get_id_type() == IDType.IO:
                node = ASTIOIDNode(id_name, id_data.get_data_type())
            else:
                node = ASTVarIDNode(id_data.get_new_name(), id_data.get_data_type())
            return node
